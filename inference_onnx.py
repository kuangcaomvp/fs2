...
class florence2Wrapper(torch.nn.Module):
    ...
    def __init__(self, device):
        ...
        self.enc_session = onnxruntime.InferenceSession('output/onnx/florence2_enc_model.onnx', providers=providers)
        self.dec_session = onnxruntime.InferenceSession('output/onnx/florence2_dec_model.onnx',
                                                        providers=providers)
    ...
    def forward(self, input_ids, pixel_values):
        ...
        encoder_outputs, attention_mask = self.enc_session.run(
            ['encoder_outputs', 'attention_mask'],
            input_feed={'input': input_ids.cpu().numpy(),
                        'pixel': pixel_values.cpu().numpy()})
        encoder_outputs = torch.tensor(encoder_outputs, device=self.device)
        ...
        while not this_peer_finished:
            ...
            lm_logits = self.dec_session.run(
                ['lm_logits'],
                input_feed={
                    'decoder_input_ids': decoder_inputs.cpu().numpy(),
                    'encoder_hidden_states': last_hidden_state.cpu().numpy(),
                    'encoder_attention_mask': encoder_attention_mask.cpu().numpy()

                })

            lm_logits = torch.tensor(lm_logits[0], device=self.device )
        ...
        torch.exp(sequence_outputs['sequence_scores'][0])
